...
class CheckouView(LoginRequiredMixin, TemplateView):
    template_name = 'checkout/checkout.html'

    # Sobrescrevendo o método get para definir como será validado o carrinho de compras do usuário
    def get(self, request, *args, **kwargs):
        ...
        if session_key and CartItem.objects.filter(cart_key=session_key).exists():
            ...
            Order.objects.create_order(user=request.user, cart_items=cart_items)
        else:
            messages.info(request, 'Não há itens no carrinho de compras')
            return redirect('checkout:cart_item')
        return super(CheckouView, self).get(request, *args, **kwargs)


# Definindo a variável da view para uso nas URLs
create_cartitem = CreateCartItemView.as_view()
cart_item = CartItemView.as_view()
checkout = CheckouView.as_view()
